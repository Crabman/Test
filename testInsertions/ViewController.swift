...
//                try db.execute(sql: "PRAGMA cipher_page_size = 16384;")
    ...
    func performInsertionsWithStatement(_ nbOfInserts:Int) {
        ...
        do{
            ...
            try testDatabase!.inTransaction { db in
                ...
                let statement = try db.makeUpdateStatement(sql: """
                                                        INSERT INTO SimpleTable ('_id', 'value1','value2','value3','value4','value5','value6','value7') VALUES (?,?,?,?,?,?,?,?)
                                        """)
                var i=0
                while i < nbOfInserts {
                    statement.setUncheckedArguments([i,1,2,3,"text",4,5,6,7])
    ...
    @IBAction func insert1millionStatementTap(_ sender: Any) {
        performInsertionsWithStatement(1000000)
    }
